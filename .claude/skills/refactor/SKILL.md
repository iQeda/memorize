...
- [x] Phase 1 — リポジトリ衛生（config / docs / i18n 死にキー）※1.2 のみ未完（下記参照）
...
- [ ] **1.2 実行不能な permission の削除** — `.claude/settings.json` から `Bash(npm run build:check)` と `Bash(pnpm lint*)` を削除（対応するスクリプトが package.json に存在しない）。テスト不要（ツール設定）。**※2026-06-12: auto モードの permission 分類器が `.claude/settings.json` の編集を拒否するため Claude からは実行不可。ユーザーが手動で 2 行削除すること。**
